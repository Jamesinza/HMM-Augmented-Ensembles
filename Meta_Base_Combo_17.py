...
for _ in range(1):
    ...
    # extra_features = get_extra_features(X_full, [6, 28, 42, 95, 138, 196, 276, 496, 8128])
    # feature_scaler = StandardScaler()
    # scaled_features = feature_scaler.fit_transform(extra_features)
    # joblib.dump(feature_scaler, f'test_models/{sub_folder}/{dataset}_feature_scaler.joblib')
    ...
    for s in range(10):
        X_sub = X_full[s*splits:splits * (s+1)]
        y_sub = data[s*splits:splits * (s+1)]
    ...
    tf.keras.backend.clear_session()
    meta1 = saving.load_model(f'test_models/{sub_folder}/Meta_L1_adamw_dim128_seed42.keras')
