...
def load_base_models(sub_folder, dataset, optimizers, dims, seeds, l=0):
    # models = []
...
def create_meta_learner(num_models, num_classes, dim, optimizer, seed, dropout, l):
    ...
    for _ in range(6):
        ...
        mha = layers.MultiHeadAttention(num_heads=2, key_dim=dim, seed=seed, kernel_initializer=initializer1)(x,x)
...
def train_meta_learner(num_classes, batch_size,
                      epochs, dataset, sub_folder, optimizers, dims, seeds, dropout,
    ...
    if base == 'base':
        ...
        base_models = load_base_models(sub_folder, dataset, optimizers, dims, seeds, l)  # Get models using generator
        ...
        tot_datasets += 1
        if tot_datasets == len(datasets) and len(datasets)>1:
            p_train = np.concatenate([p_train1, p_train], axis=1)
            p_val   = np.concatenate([p_val1, p_val], axis=1)
            p_test  = np.concatenate([p_test1, p_test], axis=1)
            train_model = True
            print(f'\nTotal datasets used so far: {tot_datasets}\n')
        else:
            if tot_datasets == len(datasets) and len(datasets)==1:
                train_model = True
            print(f'\nTotal datasets used so far: {tot_datasets}\n')            
        ...
        train_model = True
        print(f'\nTraining Meta Learners Only. All base datasets done...\n')
    ...
    if train_model:
        ...
        return model, p_train, p_val, p_test, tot_datasets
...
dropout = 0.7
num_heads = 2
seeds =  [233, 377, 610, 987, 1597, 2584, 4181]  # [1, 2, 3, 5, 8, 13, 21]  # [6, 28, 42, 95, 138, 276, 496]
dims = [128]
...
for dataset in datasets:
    ...
    print(f'\nLoaded Scaler: {dataset}_base_scaler')
    ...
    tf.keras.backend.clear_session()
    meta1, p_train, p_val, p_test, tot_datasets = train_meta_learner(
        num_classes, batch_size, epochs, dataset, sub_folder, optimizers, dims,
        seeds, dropout, X_train_reshaped, X_val_reshaped, X_test_reshaped,
        y_train, y_val, base='base', l=0, tot_datasets=tot_datasets)
    
    p_train1 = np.concatenate([p_train1, p_train], axis=1) if p_train1 is not None else p_train
    p_val1   = np.concatenate([p_val1, p_val], axis=1)     if p_val1 is not None else p_val
    p_test1  = np.concatenate([p_test1, p_test], axis=1)   if p_test1 is not None else p_test

dataset = 'Meta_L'
tf.keras.backend.clear_session()
meta2, p_train2, p_val2, p_test2, _ = train_meta_learner(
    num_classes, batch_size, epochs, dataset, sub_folder, optimizers, dims,
    seeds, dropout, p_train1, p_val1, p_test1, y_train, y_val, base='loop', l=1)
# del p_train1, p_val1, p_test1

tf.keras.backend.clear_session()
p_train12 = np.concatenate([p_train1, p_train2], axis=1)
p_val12   = np.concatenate([p_val1, p_val2], axis=1)
p_test12  = np.concatenate([p_test1, p_test2], axis=1)
meta3, p_train3, p_val3, p_test3, _ = train_meta_learner(
    num_classes, batch_size, epochs, dataset, sub_folder, optimizers, dims,
    seeds, dropout, p_train12, p_val12, p_test12, y_train, y_val, base='loop', l=2)

tf.keras.backend.clear_session()
p_train123 = np.concatenate([p_train1, p_train2, p_train3], axis=1)
p_val123   = np.concatenate([p_val1, p_val2, p_val3], axis=1)
p_test123  = np.concatenate([p_test1, p_test2, p_test3], axis=1)
meta4, p_train4, p_val4, p_test4, _ = train_meta_learner(
    num_classes, batch_size, epochs, dataset, sub_folder, optimizers, dims,
    seeds, dropout, p_train123, p_val123, p_test123, y_train, y_val, base='loop', l=3)

tf.keras.backend.clear_session()
p_train1234 = np.concatenate([p_train1, p_train2, p_train3, p_train4], axis=1)
p_val1234   = np.concatenate([p_val1, p_val2, p_val3, p_val4], axis=1)
p_test1234  = np.concatenate([p_test1, p_test2, p_test3, p_test4], axis=1)
meta5, p_train5, p_val5, p_test5, _ = train_meta_learner(
    num_classes, batch_size, epochs, dataset, sub_folder, optimizers, dims,
    seeds, dropout, p_train1234, p_val1234, p_test1234, y_train, y_val, base='loop', l=4)

tf.keras.backend.clear_session()
p_train12345 = np.concatenate([p_train1, p_train2, p_train3, p_train4, p_train5], axis=1)
p_val12345   = np.concatenate([p_val1, p_val2, p_val3, p_val4, p_val5], axis=1)
p_test12345  = np.concatenate([p_test1, p_test2, p_test3, p_test4, p_test5], axis=1)
meta6, p_train6, p_val6, p_test6, _ = train_meta_learner(
    num_classes, batch_size, epochs, dataset, sub_folder, optimizers, dims,
    seeds, dropout, p_train12345, p_val12345, p_test12345, y_train, y_val, base='loop', l=5)

tf.keras.backend.clear_session()
p_train123456 = np.concatenate([p_train1, p_train2, p_train3, p_train4, p_train5, p_train6], axis=1)
p_val123456   = np.concatenate([p_val1, p_val2, p_val3, p_val4, p_val5, p_val6], axis=1)
p_test123456  = np.concatenate([p_test1, p_test2, p_test3, p_test4, p_test5, p_test6], axis=1)
meta7, p_train7, p_val7, p_test7, _ = train_meta_learner(
    num_classes, batch_size, epochs, dataset, sub_folder, optimizers, dims,
    seeds, dropout, p_train123456, p_val123456, p_test123456, y_train, y_val, base='loop', l=6)

tf.keras.backend.clear_session()
p_train1234567 = np.concatenate([p_train1, p_train2, p_train3, p_train4, p_train5, p_train6, p_train7], axis=1)
p_val1234567   = np.concatenate([p_val1, p_val2, p_val3, p_val4, p_val5, p_val6, p_val7], axis=1)
p_test1234567  = np.concatenate([p_test1, p_test2, p_test3, p_test4, p_test5, p_test6, p_test7], axis=1)
meta8, p_train8, p_val8, p_test8, _ = train_meta_learner(
    num_classes, batch_size, epochs, dataset, sub_folder, optimizers, dims,
    seeds, dropout, p_train1234567, p_val1234567, p_test1234567, y_train, y_val, base='loop', l=7)

tf.keras.backend.clear_session()
p_train12345678 = np.concatenate([p_train1, p_train2, p_train3, p_train4, p_train5, p_train6, p_train7, p_train8], axis=1)
p_val12345678   = np.concatenate([p_val1, p_val2, p_val3, p_val4, p_val5, p_val6, p_val7, p_val8], axis=1)
p_test12345678  = np.concatenate([p_test1, p_test2, p_test3, p_test4, p_test5, p_test6, p_test7, p_test8], axis=1)
meta9, p_train9, p_val9, p_test9, _ = train_meta_learner(
    num_classes, batch_size, epochs, dataset, sub_folder, optimizers, dims,
    seeds, dropout, p_train12345678, p_val12345678, p_test12345678, y_train, y_val, base='loop', l=8)

tf.keras.backend.clear_session()
p_train123456789 = np.concatenate([p_train1, p_train2, p_train3, p_train4, p_train5, p_train6, p_train7, p_train8, p_train9], axis=1)
p_val123456789   = np.concatenate([p_val1, p_val2, p_val3, p_val4, p_val5, p_val6, p_val7, p_val8, p_val9], axis=1)
p_test123456789  = np.concatenate([p_test1, p_test2, p_test3, p_test4, p_test5, p_test6, p_test7, p_test8, p_test9], axis=1)
meta0, p_train0, p_val0, p_test0, _ = train_meta_learner(
    num_classes, batch_size, epochs, dataset, sub_folder, optimizers, dims,
    seeds, dropout, p_train123456789, p_val123456789, p_test123456789, y_train, y_val, base='loop', l=9)
...
_ = evaluate_model(meta7, p_test7, 'Meta_L7')
_ = evaluate_model(meta8, p_test8, 'Meta_L8')
_ = evaluate_model(meta9, p_test9, 'Meta_L9')
_ = evaluate_model(meta0, p_test0, 'Meta_L0')
